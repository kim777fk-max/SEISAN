...
制約: 同一machine_id + dateでアクティブなセッション（ENDED以外）は1つのみ
...
- **制約**: 同じ日付・機械で複数の独立したセッションは作成できません
- **影響**: 1日に同じ機械で異なる製品を作る場合も1つのセッションになります
- **回避策**: product_codeフィールドを活用するか、日を跨いで新しいセッションを作成してください
